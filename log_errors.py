...
for s_idx, scene_data in enumerate(test_data["scenes"]):
    ...
    for estimate_data in scene_data["estimates"]:
        ...
        for i, query_file in enumerate(pgt_poses):
            im_names.append(query_file)
            try:
                pgt_pose, rgb_focal_length = pgt_poses[query_file]
                est_pose, est_f = est_poses[query_file]
                ...
                errors[i] = math.inf
        good_res = errors < opt.error_threshold
        log_file = open(f"{estimate_data['algo']}_bad.txt", "w")
        for i, query_file in enumerate(pgt_poses):
            if not good_res[i]:
                print(query_file, errors[i], file=log_file)
